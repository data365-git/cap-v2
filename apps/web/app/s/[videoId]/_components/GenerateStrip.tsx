...
export function GenerateStrip({
	...
	const [activeLabel, setActiveLabel] = useState(
		"Xulosa, vazifalar va tahrirlangan transkript yarating",
	);
	...
	const chunkElapsedIntervalRef = useRef<ReturnType<typeof setInterval> | null>(
		null,
	);
	...
	const initialAnchorEta = useCallback(
		(phases: PipelinePhase[]) => {
		...
		(pp: PipelineProgress) => {
			...
			if (activePhase) {
				...
				const subtitleMap: Record<string, string> = {
					...
					analyze:
						"AI tahlil: Xulosa, vazifalar va tahrirlangan transkript tayyorlanmoqda…",
				};
				setPhaseSubtitle(
					subtitleMap[activePhase.key] ?? `${activePhase.label} bajarilmoqda…`,
				);
			}
				...
				try {
					...
					if (status && "aiGenerationStatus" in status) {
						...
						if (aiStatus && AI_ERROR_STATES.has(aiStatus)) {
							stopCountdown();
							// `transcriptionError` is the only failure detail the backend
							// persists. There is deliberately no AI-side error string: a failed
							// generateAiWorkflow leaves the status PROCESSING/QUEUED, which is
							// what the recover-stale-ai-jobs cron keys off to mark ERROR *and
							// re-fire* the job. Discriminating rate-limit vs. truncation here
							// would require persisting an AI error message, which must not be
							// done in the kickoff catch or that auto-retry hook is lost.
	...
	const startAiPhase = useCallback(async () => {
		try {
			...
			if (!res.ok) {
				...
				if (
					res.status === 429 ||
					/rate.limit|cost.cap/i.test(body?.error ?? "")
				) {
	...
	const kickOffPipeline = useCallback(
		async (skipTranscript: boolean) => {
			...
			try {
				const res = await fetch(`/api/videos/${videoId}/retry-transcription`, {
					method: "POST",
				});
				if (!res.ok) {
	...
	return (
		...
		>
			...
			<div className="gen-main">
				{/* Spark orb icon */}
				<div
					className={["gen-orb", isError ? "error" : ""]
						.filter(Boolean)
						.join(" ")}
					aria-hidden="true"
				>
					...
					{pipelinePhases.map((p) => {
						...
						return (
							<div
								...
								title={
									isIndexChip
										? "AI suhbat uchun (chat-da javob berish uchun ma'lumotlar tayyorlanmoqda)"
										: undefined
								}
								className={[
									"gen-chip",
							...
							>
								<div className="gen-chip-header">
									...
									{isAudioSpinner && (
										<span className="gen-chip-spinner" aria-hidden="true" />
									)}
								</div>
								{p.status === "active" && (
									<div className="gen-chip-detail">
										{isAnalyzeActive ? (
											<>
												<span className="gen-chip-shimmer-text">
													taxminan ~{ANALYZE_SEC}s
												</span>
												<ul className="gen-chip-sublist">
													{(
														[
															"Xulosa",
															"Vazifalar",
															"Tahrirlangan transkript",
															"Bo'limlar",
														] as const
													).map((item) => (
														<li key={item} className="gen-chip-subitem">
															<span
																className="gen-chip-subitem-pulse"
																aria-hidden="true"
															/>
															{item}
														</li>
											...
											<span
												className="gen-chip-spinner-inline"
												aria-hidden="true"
											/>
										) : null}
									</div>
								...
								{isAnalyzeDone && (
									<ul className="gen-chip-sublist">
										{(
											...
											<li
												key={item}
												className="gen-chip-subitem gen-chip-subitem--done"
											>
												<span
													className="gen-chip-subitem-check"
													aria-hidden="true"
												/>
												{item}
											</li>
			...
			{errorMsg && phase === "running" && (
				<p className="gen-error">{errorMsg}</p>
			)}
		</div>
	);
}
